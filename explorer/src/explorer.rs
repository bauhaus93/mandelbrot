...
impl Explorer {
    pub fn new(screen_size: [i32; 2], update_frequency: i32) -> Result<Explorer, ExplorerError> {
        ...
        match core.install_keyboard() {
            Ok(e) => e,
            Err(_) => return Err(ExplorerError::Allegro("Could not install keyboard".to_string()))
        }

        let display = match Display::new(&core, screen_size[0], screen_size[1]) {
            Ok(e) => e,
            Err(_) => return Err(ExplorerError::Allegro("Could not create display".to_string()))
        };

        let event_queue = match EventQueue::new(&core) {
            Ok(e) => e,
            Err(_) => return Err(ExplorerError::Allegro("Could not create event queue".to_string()))
        };

        let update_timer = match Timer::new(&core, 1.0 / update_frequency as f64) {
            Ok(e) => e,
            Err(_) => return Err(ExplorerError::Allegro("Could not update create timer".to_string()))
        };
        ...
        match core.get_keyboard_event_source() {
            ...
            None => return Err(ExplorerError::Allegro("No keyboard event source".to_string()))
        }
    ...
    fn handle_keydown(&mut self, key: allegro::KeyCode) {
        ...
        match key {
            ...
            allegro::KeyCode::F1 => {
                ...
                info!("Starting snapshot of size {}x{}", snapshot_shape[0], snapshot_shape[1]);
